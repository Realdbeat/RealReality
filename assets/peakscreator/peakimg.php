...
*/
  if (file_exists($mp3file)) {
    ...
    // temporary file name
    $tmpname = urldecode( $upload_dir['path'] . '/' . substr(md5(time()), 0, 10));
    
    // copy from temp upload directory to current
    copy($mp3file, "{$tmpname}.mp3");
     ...
     */
    exec("{$lamep}lame {$tmpname}.mp3 -m m -S -f -b 16 --resample 8 $mp3file  &&  {$lamep}lame -S --decode $mp3file {$tmpname}.wav");
      $wavs_to_process[] = "{$tmpname}.wav";
 
    exec("{$lamep}lame –decode $mp3file {$tmpname}.wav");     
 ...
 wp_die();
    ...
    wp_send_json_error("file not pass well");  
    wp_die();
  }    