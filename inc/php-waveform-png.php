...
$mainpath =  ABSPATH."/wp-content/uploads/".$_POST["id"].".mp3";
  ...
  if (isset($mainpath)) {
    ...
    for($wav = 1; $wav <= sizeof($wavs_to_process); $wav++) {
      ...
      if (!$img) {
        ...
        $img = imagecreatetruecolor($data_size / DETAIL, $height * sizeof($wavs_to_process)) or die('Cannot Initialize new GD image stream');
