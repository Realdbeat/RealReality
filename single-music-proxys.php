...
$id = isset($_POST["id"]) ? $_POST["id"] : "snakemusics";
$mainfile =  PEAKS_PATH.$id.".mp3";
$lamep =  PEAKS_PATH;
 /*
...
*/
  if (isset($id)) {
    ...
    // temporary file name
    $tmpname =  PEAKS_PATH.substr(md5(time()), 0, 10);
    
    // copy from temp upload directory to current
    copy($mainfile, "{$tmpname}.mp3");
    $file = fopen ("{$tmpname}.mp3" , 'rb') or die("Unable to open file!");
    ...
    if ($stereo) {
      ...
      exec("lame {$tmpname}.mp3 --scale-r 0.1 -m m -S -f -b 16 --resample 8 {$mainfile}  && lame -S --decode {$tmpname}.mp3 {$tmpname}_l.wav");
			// same as above, left channel
      exec("lame {$tmpname}.mp3 --scale-l 0.1 -m m -S -f -b 16 --resample 8 {$mainfile} && lame -S --decode {$mainfile}  {$tmpname}_r.wav");
      $wavs_to_process[] = "{$tmpname}_l.wav";
      $wavs_to_process[] = "{$tmpname}_r.wav";
    } else {
      exec("{$lamep}lame {$tmpname}.mp3 -m m -S -f -b 16 --resample 8 $mainfile  && {$lamep}lame -S --decode $mainfile {$tmpname}.wav");
      $wavs_to_process[] = "{$tmpname}.wav";}
 
exec("lame –decode $mainfile {$tmpname}.wav");     
    ...
    imagedestroy($img);
 echo "ALL DONE AND LINK IS ".get_path($save);
  } else {
